...
export function DashboardPage(){const[s,setStats]=useState<DashboardStats|null>(null);const[tl,setTpL]=useState<string[]>([]);const[tv,setTpV]=useState<number[]>([]);const[svc,setSvc]=useState<ServiceHealth|null>(null);const[log,setLog]=useState<string[]>(['[SYS] Online.']);const lr=useRef<HTMLDivElement>(null);
const ld=useCallback(async()=>{try{const[sr,tp,h]=await Promise.all([getStats(),getThroughput(),getHealth()]);setStats(sr);setTpL(tp.labels);setTpV(tp.values);setSvc(h.services as ServiceHealth);setLog(p=>[...p,`[${new Date().toLocaleTimeString()}] ${sr.total} mems | ${sr.active_users} users`].slice(-50))}catch{ /* API unavailable, silent */ }},[]);
...
return(<div><div className='page-title'>控制台</div><div className='page-sub'>实时系统状态监控</div>
<div className='stats-grid'><StatCard color='teal' label='全局记忆' value={s?.total?.toLocaleString()??'—'} sub={s?.memory_growth??'加载中...'}/><StatCard color='violet' label='活跃租户' value={s?.active_users?.toLocaleString()??'—'} sub='注册租户总数'/><StatCard color='emerald' label='今日写入' value={s?.today_writes?.toLocaleString()??'—'} sub='实时写入频率'/><StatCard color='amber' label='已省 Token' value={s?.tokens_saved?.toLocaleString()??'—'} sub='全局 RAG 减免'/></div>
