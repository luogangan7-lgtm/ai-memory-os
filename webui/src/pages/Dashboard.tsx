...
const ld=useCallback(async()=>{try{const[sr,tp,h]=await Promise.all([getStats(),getThroughput(),getHealth()]);setStats(sr);setTpL(tp.labels);setTpV(tp.values);setSvc(h.services as ServiceHealth);setLog(p=>[...p,`[${new Date().toLocaleTimeString()}] ${sr.total} mems | ${sr.active_users} users`].slice(-50))}catch{}},[toast]);
