...
function TokenPanel(){
const [token]=useState('mos_xxxx_xxxxxxxxxxxxxxxx');const [copied,setCopied]=useState(false);
return(<div className='card'><div className='card-title'>MCP 接入 Token</div><div style={{display:'flex',gap:8,alignItems:'center'}}><code style={{flex:1,background:'rgba(0,0,0,.3)',padding:'10px 14px',borderRadius:8,fontSize:12,fontFamily:'var(--mono)',wordBreak:'break-all'}}>{token}</code><button className='btn btn-teal' onClick={()=>{navigator.clipboard.writeText(token);setCopied(true);setTimeout(()=>setCopied(false),2000)}}>{copied?'已复制':'复制'}</button></div><div style={{marginTop:12,fontSize:11,color:'var(--muted)'}}>将此 Token 填入 Agent 的 MCP 配置即可接入记忆系统</div></div>)}

export function UserAppPage(){
const [tab,setTab]=useState<'chat'|'memory'|'token'>('chat');
return(<div style={{maxWidth:900,margin:'0 auto',padding:'40px 24px'}}><div style={{textAlign:'center',marginBottom:32}}><div className='logo-orb' style={{margin:'0 auto 16px',width:56,height:56,fontSize:26,borderRadius:16}}>🧠</div><div className='page-title' style={{textAlign:'center'}}>我的记忆空间</div><div className='page-sub' style={{textAlign:'center'}}>AI 对话 · 记忆管理 · Token 接入</div></div><div style={{display:'flex',gap:10,justifyContent:'center',marginBottom:24}}>{(['chat','memory','token'] as const).map(t=><button key={t} className={`btn ${tab===t?'btn-teal':'btn-ghost'}`} onClick={()=>setTab(t)}>{t==='chat'?'💬 AI 对话':t==='memory'?'🧠 我的记忆':'🔑 Token'}</button>)}</div>{tab==='chat'&&<ChatPanel/>}{tab==='memory'&&<MemoryPanel/>}{tab==='token'&&<TokenPanel/>}</div>)}
